...
tinymce.PluginManager.add( 'wpview', function( editor ) {
	...
	function expand( value ) {
		...
		while ( i-- ) {
			expanded[ cssExpand[ i ] ] = parseInt( parts[ i ] || parts[ i - 2 ] || parts[ 0 ], 10 );
		}
	...
	editor.on( 'click', function( event ) {
		var x = event.clientX,
			y = event.clientY,
			body = editor.getBody(),
			bodyPadding = expand( editor.$( body ).css( 'padding' ) ),
		...
		if ( y < firstRect.top && ( view = getView( first ) ) ) {
			...
			event.preventDefault();
		} else if ( x < bodyRect.left + bodyPadding.left || x > bodyRect.right - bodyPadding.right ) {
			tinymce.each( editor.dom.select( '.wpview-wrap' ), function( view ) {
				...
				if ( y >= rect.top && y <= rect.bottom ) {
					if ( x < bodyRect.left + bodyPadding.left ) {
						setViewCursor( true, view );
						event.preventDefault();
					} else if ( x > bodyRect.right - bodyPadding.right ) {
						setViewCursor( false, view );
						event.preventDefault();
					}
