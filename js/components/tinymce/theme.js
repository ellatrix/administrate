...
tinymce.ThemeManager.add( 'modern', function( editor ) {
	...
	var defaultToolbar = [ 'bold', 'italic', 'strikethrough', 'bullist', 'numlist', 'blockquote', 'hr', 'alignleft', 'aligncenter', 'alignright', 'link', 'unlink' ].join( ' | ' );
	...
	function renderInlineUI(args) {
		...
		if (settings.fixed_toolbar_container) {
			if ( typeof settings.fixed_toolbar_container === 'string' ) {
				inlineToolbarContainer = DOM.select(settings.fixed_toolbar_container)[0];
			} else {
				inlineToolbarContainer = settings.fixed_toolbar_container
			}
		}
		...
		function render() {
			...
			editor.on('blur deactivate', function() {
